...
import { TransactionCard } from '@/components/shared/TransactionCard' // Bug #1
...
export function DashboardPage() {
  ...
  // Only count actual txns for inflow/outflow (record txns are not settled)
  const recentInflow  = recentActivity
    .filter(t => Number(t.amount) > 0 && t.type === 'actual')
      ...
      <section className="grid grid-cols-3 gap-3">
        <Card className="rounded-xl shadow-sm">
          <CardContent className="p-3 text-center">
            ...
            <p className="text-[10px] text-muted-foreground font-medium">Money In</p>
            <p className="text-sm font-bold text-emerald-600 mt-0.5">
              {fmtAmount(recentInflow)}
            </p>
          </CardContent>
        </Card>
        <Card className="rounded-xl shadow-sm">
          <CardContent className="p-3 text-center">
            ...
            <p className="text-[10px] text-muted-foreground font-medium">Money Out</p>
            <p className="text-sm font-bold text-red-600 mt-0.5">
              {fmtAmount(recentOutflow)}
            </p>
          </CardContent>
        </Card>
        ...
        <div className="space-y-1">
          {recentActivity.length === 0 ? (
            ...
            recentActivity.slice(0, 5).map(txn => (
              // Bug #1: Use shared TransactionCard
              // Bug #17: UX improved — arrows, colors, labels ("Received/Sent" not "actual/record")
              // showContact=true since dashboard shows all contacts
              <TransactionCard
                key={txn.id}
                txn={txn}
                showContact
