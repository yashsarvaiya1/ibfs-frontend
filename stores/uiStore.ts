...
interface UIStore {
  ...
  // ── Move Stock Sheet ─────────────────────────────────────────────────────────
  // source: 'document' = from doc page, 'product' = from product page
  ...
  // Persisted — survives navigation. '' = All, 'record' = Expected,
  // 'actual' = Settled, 'contra' = Contra
  ...
  persist(
    (set) => ({
      ...
      openQuickAction: (type) =>
        set({ quickActionOpen: true, quickActionType: type }),
      closeQuickAction: () =>
        set({ quickActionOpen: false, quickActionType: null }),
      ...
      openMoveStockSheet: (documentId, source = "document") =>
        set({
          moveStockSheetOpen: true,
          moveStockDocumentId: documentId,
          moveStockSource: source,
        }),
      closeMoveStockSheet: () =>
        set({
          moveStockSheetOpen: false,
      ...
      deleteDocId: null,
      openDeleteDocSheet: (id) =>
        set({ deleteDocSheetOpen: true, deleteDocId: id }),
      closeDeleteDocSheet: () =>
        set({ deleteDocSheetOpen: false, deleteDocId: null }),
      ...
      openAdjustBalanceSheet: (accountId) =>
        set({
          adjustBalanceSheetOpen: true,
          adjustBalanceAccountId: accountId,
        }),
      ...
      openAdjustStockSheet: (productId, mode = "add") =>
        set({
          adjustStockSheetOpen: true,
          adjustStockProductId: productId,
          adjustStockMode: mode,
        }), // ← update
    ...
    }),
    {
      ...
      partialize: (state) => ({
        ...
        globalTxnFilter: state.globalTxnFilter, // persisted — survives navigation
      }),
    },
  ),
);
